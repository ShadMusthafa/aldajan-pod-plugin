sap.ui.define(
  ...
  function (JSONModel, PluginViewController, Log, MessageBox, ErrorHandler, formatter) {
    ...
    var oPluginViewController = PluginViewController.extend(
      'arun.ext.podplugins.massOperatorAssignmentPluginV2.controller.PluginView',
      {
        metadata: {
          properties: {}
        },
     ...
     */
        ...
        onAssignedResourceChanged: async function (oEvent) {
          ...
          /////////////////////////
          var aLineItems = oViewModel.getProperty("/lineItems"), dupComponent;
        ...
        onAssignedOperatorIdChange: function (oEvent) {
          var oControl = oEvent.getSource(), dupComponent, resource;
     ...
     */
        ...
        _getSfcData: function () {
          ...
          var oParamters = {
            plant: this.getPodController().getUserPlant(),
            sfc: this.getView().byId("idSFCSelect").getSelectedKey()
          };
        ...
        _getOrderDetails: function (sOrderId) {
          // var sUrl = 'https://api.test.eu20.dmc.cloud.sap/order/v1/orders';
        ...
        _handleMaterialDataFetch: async function (aMaterials) {
          ...
          aLineItems = aLineItems.reduce((acc, oItem) => {
            if (oItem.isUnitValid) acc.push(oItem);
            return acc;
          }, []);

          this._getAssignmentDataHANADB().then(aData => {
            if (aData.length > 0) {
              var obj;
              for (var i in aLineItems) {
                obj = aData.find(oItem => {
                  return oItem.COMPONENT === aLineItems[i].component && oItem.SEAT_NUMBER === aLineItems[i].InSeatNumber
                });
                aLineItems[i].InSeatNumber = obj.SEAT_NUMBER;
                aLineItems[i].resource = obj.RESOURCE;
                aLineItems[i].resourceType = obj.RESOURCE ? aLineItems[i].resourceType : "";
                aLineItems[i].isNew = obj.RESOURCE ? aLineItems[i].isNew : true;
                aLineItems[i].operator = obj.OPERATOR;
                aLineItems[i].acceptanceDelay = obj.ACCEPTANCE_DELAY;
                aLineItems[i].correctionTime = obj.CORRECTION_TIME;
                aLineItems[i].lastModified = obj.UPDATED_DATE_TIME;
                aLineItems[i].InActive = obj.ACTIVE;
              }

              // Find elements in array1 that are not in array2
              let difference = aData.filter(obj1 => 
                !aLineItems.some(obj2 => obj2.component === obj1.COMPONENT && obj2.InSeatNumber === obj1.SEAT_NUMBER)
              );
            ...
            // if (this.getView().getModel("orderData").getData().status === "COMPLETED") {
            //   MessageBox.error("Selected order is already executed.");
            //   return;
            // }
        ...
        _createTableLineItems: function (aData) {
          var oConfiguration = this.getConfiguration(), InSeatNumber = 0;
          ...
          aRecipeItems.forEach(oItem => {
            ...
            aResources.forEach((oResource, index) => {
              ...
              oLineItem.InSeatNumber = oLineItem.InSeatNumber + index
              if (oResource.asset) {
